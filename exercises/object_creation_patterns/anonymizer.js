...
// user objects on init. The created object should not have
// access to the function that anonymizes a user other than
// through the init and reanonymize methods. The function that
// anonymizes creates a 16 character sequence composed of letters
// and numbers. The following are the properties and methods on
// the Account object:

const Account = {
  init(email, password, firstName, lastName) {
    this.email = function (pwAttempt) {
    ...
    this.resetPassword = function (pwAttempt, newPassword) {
      if (pwAttempt === password) {
    ...
    this.firstName = function (pwAttempt) {
      if (pwAttempt === password) {
        return firstName;
      } else {
        console.log('Invalid Password');
    ...
    this.lastName = function (pwAttempt) {
      if (pwAttempt === password) {
        return lastName;
      } else {
        console.log('Invalid Password');
    ...
    this.reanonymize = function (pwAttempt) {
      if (pwAttempt === password) {
    ...
    function anonymize() {
      ...
      const randomArray = [];
      for (let i = 0; i <= 15; i++) {
        const randomIdx = (chars.length * Math.random()).toFixed(0);
        randomArray.push(chars[randomIdx]);
      }
...
// resetPassword: This method asks the user for a new password
// and reassigns it to the password property. To reset the
// password, the user must provide the current password. Returns
// 'Invalid Password' if the password provided is not
